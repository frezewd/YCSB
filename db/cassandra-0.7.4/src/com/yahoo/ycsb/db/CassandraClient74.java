 ...
 */
public class CassandraClient74 extends DB
{
...
System.out.println("hi2");
   try
	{ 	
		List<KsDef>lks = client.describe_keyspaces();
		boolean utexist = false;
		for(KsDef ks:lks){
			System.out.println(ks.getName());
			if(ks.getName().equals("usertable")) utexist=true;
		}
//System.out.println(utexist);	
		if (!utexist){
			KsDef ksd = new KsDef();
			ksd.setName("usertable");
			ksd.setReplication_factor(1);
			ksd.setStrategy_class("org.apache.cassandra.locator.SimpleStrategy");
    			CfDef cfd = new CfDef();
			cfd.setName("data");
			cfd.setKeyspace("usertable");
			List<CfDef> lcf = new ArrayList<CfDef>();
			lcf.add(cfd);
			ksd.setCf_defs(lcf);
//System.out.println("hi3- from inside");
			//System.out.println(client.describe_version());
			client.system_add_keyspace(ksd);
//System.out.println("hi4- from inside");
		}
	}catch(InvalidRequestException e){      e.printStackTrace();      e.printStackTrace(System.out);}
	 catch(TException e){      e.printStackTrace();      e.printStackTrace(System.out);}
System.out.println("hi5");
  ...
  public int read(String table, String key, Set<String> fields, HashMap<String, String> result)
  {
    ...
    for (int i = 0; i < OperationRetries; i++)
    {
	  ...
	  //List<ByteBuffer> fieldlist = new List<ByteBuffer>(fields.size());
          for (String s : fields)
          {
        ...
        for (ColumnOrSuperColumn oneresult : results)
        {
          ...
          result.put(new String(column.name.toString()), new String(column.value.toString()));

          if (_debug)
          {
            System.out.print("(" + new String(column.name.toString()) + "=" + new String(column.value.toString()) + ")");
          }
        }
      ...
      try
      {
        ...
        for (KeySlice oneresult : results)
        {
          ...
          for (ColumnOrSuperColumn onecol : oneresult.columns)
          {
            ...
            tuple.put(new String(column.name.toString()), new String(column.value.toString()));

            if (_debug)
            {
              System.out
                  .print("(" + new String(column.name.toString()) + "=" + new String(column.value.toString()) + ")");
            }
          }
  ...
  public int insert(String table, String key, HashMap<String, String> values)
  {
    ...
    for (int i = 0; i < OperationRetries; i++)
    {
	  ...
	  col.setName(field.getBytes("UTF-8"));
	  col.setValue(val.getBytes("UTF-8"));
  ...
  {
    CassandraClient74 cli = new CassandraClient74();
